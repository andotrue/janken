<?xml version="1.0" encoding="UTF-8" ?>
<Package name="janken" format_version="4">
    <Manifest src="manifest.xml" />
    <BehaviorDescriptions>
        <BehaviorDescription name="behavior" src="." xar="behavior.xar" />
    </BehaviorDescriptions>
    <Dialogs />
    <Resources>
        <File name="base" src="html/css/base.css" />
        <File name="common" src="html/css/common.css" />
        <File name="style" src="html/css/style.css" />
        <File name="aiko" src="html/images/aiko.png" />
        <File name="bg" src="html/images/bg.jpg" />
        <File name="bg_title" src="html/images/bg_title.png" />
        <File name="choki" src="html/images/choki.png" />
        <File name="exit" src="html/images/exit.png" />
        <File name="gameover" src="html/images/gameover.jpg" />
        <File name="goo" src="html/images/goo.png" />
        <File name="jan" src="html/images/jan.png" />
        <File name="ken" src="html/images/ken.png" />
        <File name="loading" src="html/images/loading.png" />
        <File name="p_choki" src="html/images/p_choki.png" />
        <File name="p_goo" src="html/images/p_goo.png" />
        <File name="par" src="html/images/par.png" />
        <File name="parts_title" src="html/images/parts_title.png" />
        <File name="yarareta" src="html/images/yarareta.png" />
        <File name="zannen" src="html/images/zannen.png" />
        <File name="index" src="html/index.html" />
        <File name="base" src="html/js/base.js" />
        <File name="core" src="html/js/core.js" />
        <File name="janken" src="html/js/janken.js" />
        <File name="jquery-1.8.3.min" src="html/js/jquery-1.8.3.min.js" />
        <File name="gameclear" src="html/images/gameclear.jpg" />
        <File name="gameover" src="html/images/gameover.png" />
        <File name="gameclear" src="html/images/gameclear.png" />
        <File name="pepper" src="html/js/pepper.js" />
        <File name="p_par" src="html/images/p_par.png" />
    </Resources>
    <Topics />
    <IgnoredPaths>
        <Path src="html/images/P_par.png" />
        <Path src="html/images/p_par.png" />
    </IgnoredPaths>
</Package>
